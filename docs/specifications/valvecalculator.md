...
| BR-05 | **Агрегация результатов** | Если в расчете несколько клапанов одного типа (напр., 1 СК + 3 РК Тип-1 + 1 РК Тип-2), формируются **сводные таблицы**:<br>1. Суммарно по всем СК.<br>2. Суммарно по каждой из геометрий клапанов. |
