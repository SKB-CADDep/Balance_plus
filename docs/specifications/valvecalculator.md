...
| BR-05 | **Агрегация результатов** | Если в расчете несколько клапанов одного типа (напр., 1 СК + 3 РК Тип-1 + 1 РК Тип-2), формируются **сводные таблицы**:<br>1. Суммарно по всем СК.<br>2. Суммарно по каждой из геометрий клапанов. Т.е для данного примера всего будет 3 таблицы (1 СК + РК Тип1 + РК Тип 2).|
...
| Температура воздуха| `T_air` | °С | Single |  |
| Давления воздуха | `P_air` | кгс/см² | Single |  |
| Давления последнего отсоса | `P_lst_leak_off` | кгс/см² | Single | Т.к. оно одинаково для всей всех клапанов части |
