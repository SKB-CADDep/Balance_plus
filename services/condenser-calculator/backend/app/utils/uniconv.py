...
) -> tuple[Callable[[Number], Number], Callable[[Number], Number]]:
    """
...
class UnitConverter:
    ...
    # ------------------------ PUBLIC -----------------------------
    def convert(
        self, value: Number, *, from_unit: str, to_unit: str, parameter_type: str
    ) -> float:
        """
        ...
        base_val = self.to_base(
            value, from_unit=from_unit, parameter_type=parameter_type
        )
        return self.from_base(base_val, to_unit=to_unit, parameter_type=parameter_type)
    ...
    def add_parameter(
        self, parameter_type: str, *, base_unit_symbol: str, base_unit_name: str
    ) -> None:
    ...
    def add_unit(
        ...
        *,
        ...
        from_base: FactorOrFunc | None = None,
    ) -> None:
        """
    ...
    def _build_defaults(self) -> None:
        ...
        # 2) Temperature --------------------------------------------
        self.add_parameter(
            "temperature", base_unit_symbol="°C", base_unit_name="градус Цельсия"
        )
        ...
        # 7) Power ---------------------------------------------------
        self.add_parameter("power", base_unit_symbol="МВт", base_unit_name="мегаватт")
        ...
        # 8) Mass flow rate -----------------------------------------
        self.add_parameter(
            "mass_flow", base_unit_symbol="т/ч", base_unit_name="тонна в час"
        )
        ...
        # 9) Heat power (Q) -----------------------------------------
        self.add_parameter(
            "heat_power", base_unit_symbol="Гкал/ч", base_unit_name="гигакалория в час"
        )
        ...
        self.add_parameter(
            "quality", base_unit_symbol="fraction", base_unit_name="доля (0–1)"
        )
...
if __name__ == "__main__":
    ...
    t_kelvin = uc.convert(
        100, from_unit="°C", to_unit="K", parameter_type="temperature"
    )
    print(f"100 °C = {t_kelvin:.2f} K")

    # 3. Плотность: 1.2 г/см³ → кг/м³
    rho = uc.convert(1.2, from_unit="г/см³", to_unit="кг/м³", parameter_type="density")
    ...
    hp = uc.convert(2500, from_unit="кВт", to_unit="л.с.", parameter_type="power")
