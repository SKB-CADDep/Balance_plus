...
class MetroVickersStrategy:
    ...
    def calculate(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        length_cooling_tubes_of_the_main_bundle = params[
            "length_cooling_tubes_of_the_main_bundle"
        ]
        number_cooling_tubes_of_the_main_bundle = params[
        ...
        diameter_outside_of_pipes = (
            diameter_inside_of_pipes + 2 * thickness_pipe_wall
        )  # p.1
        area_tube_bundle_surface_total = (
            math.pi
            * length_cooling_tubes_of_the_main_bundle
            * (
                number_cooling_tubes_of_the_main_bundle
        ...
        coefficient_R1 = (
            2 * thickness_pipe_wall / 1000 * diameter_outside_of_pipes / 1000
        ) / (
        ...
        coefficient_K_temp = self._get_k_from_table_temp(
            # p.5
            (speed_cooling_water_const, temperature_cooling_water_average_heating_const)
        ).item()

        speed_cooling_water = (
            mass_flow_cooling_water * number_cooling_water_passes_of_the_main_bundle
        ) / (
            900
            * math.pi
            * (
        ...
        temperature_cooling_water_2 = (
            temperature_cooling_water_1 + delta_t_water
        )  # p.10
        temperature_cooling_water_average_heating = (
            temperature_cooling_water_1 + temperature_cooling_water_2
        ) / 2  # p.11
        ...
        k_clean_denominator = (
            (1 / (coefficient_K_temp * 0.85 * coefficient_B_const * coefficient_Kf))
            - 0.087 / 10000
            + coefficient_R1
        )  # p.12
        ...
        temperature_relative_underheating = 1 / (
            math.e
            ** (
                (coefficient_Kzag * area_tube_bundle_surface_total)
                / (mass_flow_cooling_water * 1000)
            )
            - 1
        )  # p.14
        ...
        temperature_saturation_steam = (
            temperature_cooling_water_2
            + temperature_relative_underheating
            * (temperature_cooling_water_2 - temperature_cooling_water_1)
        )  # p.15

        pressure_flow_path_1_mpa = seuif97.tx(temperature_saturation_steam, 1.0, 1)
        ...
        results.update(
            {
                ...
                or speed_cooling_water > max(k_interpolation_data["speed_points"])
                or temperature_cooling_water_average_heating
                > max(k_interpolation_data["temperature_points"]),
            }
        )
