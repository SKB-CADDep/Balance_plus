"""
Математическое ядро расчета конденсаторов по методике ВТИ (Бермана).

Основано на нормативном методе Л.Д. Бермана. Применяет паттерн Strategy (Стратегия) 
для выполнения пакетных (batch) расчетов. Ядро принимает массивы входных данных 
(расходы, температуры) и генерирует полный набор результатов (плоский список) 
за один проход, используя внутренние матрицы.
"""
...
class BermanStrategy:
    """
    Стратегия расчета абсолютного давления пара в конденсаторе (P_steam).

    Методика предназначена для расчета давления за последней ступенью турбины 
    (без учета гидравлических потерь в переходном патрубке) на основе геометрии 
    конденсатора и параметров охлаждающей воды. 
    
    Ограничения метода: 
    Расчет ведется согласно нормативному методу Л.Д. Бермана для температур 
    охлаждающей воды до 45°С. 
    
    Алгоритм поддерживает сложную топологию: учитывает наличие двух пучков 
    трубок (основного ОП и встроенного ВП), балансируя их тепловые потоки 
    с помощью внутреннего численного решателя.
    """

    def calculate(self, params: dict) -> dict:
        """
        Выполняет основной пакетный расчет теплообмена и давления.

        Args:
            params (dict): Словарь с подготовленными физическими параметрами.
                Ожидаемые ключи:
                - Геометрия: 'L_main', 'L_builtin', 'Z_main', 'Z_builtin', 'N_main', 'N_builtin', 'd_in', 'S_tube'
                - Термодинамика: 'H_steam', 'G_nom', 'lambda' (уточненная теплопроводность)
                - Массивы режимов: 'W_main_list', 'W_builtin_list', 't1_main_list', 't1_builtin_list', 'G_steam_list', 'coefficient_b_list'
                - Прочее: 'G_air' (расход присосов воздуха).

        Returns:
            dict: Словарь с двумя ключами:
                - 'main_results' (list[dict]): Плоский список результатов теплового расчета 
                  для каждой перестановки входных массивов. Содержит промежуточные 
                  коэффициенты (Phi_c, Phi_t, K_clean и др.) и финальные давления (P_steam).
                - 'ejector_results' (list[dict]): Данные расчета эжекторной установки.
        """
        ...
        # Заполнение матриц из входных списков 
        # (Используется 1-индексация для совместимости с историческими формулами пучков: 1-ОП, 2-ВП)
        ...
        # --- 3. Основной вычислительный цикл (Batch processing) ---
            ...
            for m in range(1, len(coefficient_b_list) + 1):
                ...
                for j in range(1, max_temp_idx + 1):
                    ...
                    for steam_idx in range(1, max_steam_flow_idx + 1):
                        ...
                        # Численный решатель (балансировка температур для двухпучковых аппаратов)
                        if use_two_bundles:
                        ...
                        try:
                            # Аппроксимация давления по температуре
                            ...
                            # Точный расчет давления по библиотеке seuif97 (вода/пар)
