...
class CalculationInput(BaseModel):
    ...
    G_steam: list[float] = Field(
        ..., min_length=1, description="Массив расходов пара (Ось X)"
    )
    W_main: list[float] = Field(
        ..., min_length=1, description="Массив расходов основной охл. воды"
    )
    W_builtin: list[float] | None = Field(
        default=None, description="Массив расходов воды встроенного пучка"
    )
    t1_main: list[float] = Field(
        ..., min_length=1, description="Массив температур воды на входе (Ось Y)"
    )
    t1_builtin: list[float] | None = Field(
    ...
    Z_builtin: int | None = Field(
        default=None, ge=1, description="Число ходов встроенного пучка"
    )

    # Термодинамика пара
    H_steam: float | None = Field(
        default=None, description="Энтальпия пара (Обязательно для Бермана)"
    )
    X_steam: float = Field(
        default=0.950, le=1.0, description="Степень сухости пара (Метро-Виккерс)"
    )
    ...
    def validate_cross_dependencies(self) -> "CalculationInput":
        ...
        if self.method == "berman" and self.H_steam is None:
            raise ValueError(
                "Для метода 'berman' параметр энтальпии (H_steam) является обязательным."
            )
...
class MatrixResult(BaseModel):
    ...
    meta: dict[str, Any] = Field(
        ..., description="Метаданные (какие W и b использовались для этой матрицы)"
    )
    ...
    values: list[list[float]] = Field(
        ..., description="Матрица значений давления (P_steam)"
    )
    warnings: list[str] = Field(
        default_factory=list,
        description="Предупреждения (выход за диапазоны, экстраполяция)",
    )
...
class CalculationOutput(BaseModel):
    ...
    tables: list[MatrixResult] = Field(
        ..., description="Сгенерированные матрицы P_steam"
    )
    ejector_results: list[EjectorResult] = Field(
        default_factory=list, description="Данные эжекторов"
    )
