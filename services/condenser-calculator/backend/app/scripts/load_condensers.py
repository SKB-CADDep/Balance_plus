...
def load_condensers(db: Session, excel_path: str = "default.xlsx"):
    ...
    for index, row in df.iterrows():
        ...
        if condenser_name in processed_names:
            logger.info(
                f"Конденсатор {condenser_name} уже обработан. Пропускаем дубликат."
            )
            continue
        ...
        if raw_material_string and raw_material_string != "nan":
            ...
            material_names = [
                name.strip() for name in raw_material_string.split("/") if name.strip()
            ]

            for mat_name in material_names:
                material_obj = (
                    db.query(Material)
                    .filter(Material.name.ilike(f"%{mat_name}%"))
                    .first()
                )
                if material_obj:
        ...
        if not matched_materials:
            ...
            if default_material:
                ...
                logger.warning(
                    f"Для {condenser_name} не найдено совпадений. Привязан default материал."
                )
        ...
        water_limits = {
            ...
            "total_max": safe_float(
                row.get(
                    "Максимальный_расход_охлаждающей_воды_через_основные_и_встроенные_пучки_т/ч"
                )
            ),
        }
        ...
        new_condenser = Condenser(
            ...
            doc_num_thermo_calc=str(
                row.get("Теплогидравлический_расчет_конденсатора_(номер_документа)", "")
            ),
            ...
            builtin_count=safe_int(
                row.get("Количество_охлаждающих_труб_встроенного_пучка_шт")
            ),
            aircooler_count=safe_int(
                row.get("Общее_количество_труб_воздухоохладителя_шт")
            ),
