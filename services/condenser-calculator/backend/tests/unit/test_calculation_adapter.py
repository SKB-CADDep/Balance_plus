...
def create_mock_matrix(
    warnings: list[str] | None = None, meta: dict | None = None
) -> MatrixResult:
...
def test_berman_warning_water_flow_limits(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
) -> None:
    ...
    with patch.object(adapter, "_run_berman") as mock_run:
        mock_run.return_value = (
            [
                create_mock_matrix(
                    warnings=[
                        "Расход охлаждающей воды превышает максимальный паспортный"
                    ]
                )
            ],
            [],
...
def test_berman_warning_temperature_range_high(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
    ...
    with patch.object(adapter, "_run_berman") as mock_run:
        mock_run.return_value = (
            [
                create_mock_matrix(
                    warnings=[
                        "Температура 50.0°C выходит за оптимальный диапазон Бермана"
                    ]
                )
            ],
            [],
...
def test_metrovickers_warning_extrapolation(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_metrovickers: CalculationInput,
) -> None:
    ...
    with patch.object(adapter, "_run_metrovickers") as mock_run:
        mock_run.return_value = [
            create_mock_matrix(
                warnings=[
                    "Данные находятся в области экстраполяции кривой K (Метро-Виккерс)"
                ]
            )
        ]
...
def test_w_builtin_empty_array(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
...
def test_different_lengths_w_main_w_builtin(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
...
def test_empty_coefficient_b_uses_default(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
...
def test_engine_errors_are_wrapped(
    adapter: CondenserCalculationAdapter,
    mock_condenser: MagicMock,
    mock_material: MagicMock,
    input_berman: CalculationInput,
    ...
    with patch.object(
        adapter,
        "_run_berman",
        side_effect=Exception("Internal physics division by zero"),
    ):
        with pytest.raises(
            CalculationEngineError, match="Ошибка при выполнении расчёта"
        ):
            adapter.calculate(input_berman, mock_condenser, mock_material)
