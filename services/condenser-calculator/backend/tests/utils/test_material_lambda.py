...
def valid_material() -> MockMaterial:
    return MockMaterial(
        "МНЖ 5-1",
        [MockPoint(20.0, 15.0), MockPoint(100.0, 35.0), MockPoint(150.0, 45.0)],
    )
...
def unordered_material() -> MockMaterial:
    return MockMaterial(
        "Латунь Л68",
        [MockPoint(100.0, 35.0), MockPoint(20.0, 15.0), MockPoint(150.0, 45.0)],
    )
...
def test_get_lambda_exact_point(
    mock_table: MagicMock, valid_material: MockMaterial
) -> None:
...
@patch("app.core.material_lambda.Table1D", side_effect=FakeTable1D)
def test_get_lambda_interpolation(
    mock_table: MagicMock, valid_material: MockMaterial
) -> None:
    interp = build_lambda_interpolator(valid_material)
...
@patch("app.core.material_lambda.Table1D", side_effect=FakeTable1D)
def test_get_lambda_unordered_points(
    mock_table: MagicMock, unordered_material: MockMaterial
) -> None:
    interp = build_lambda_interpolator(unordered_material)
    assert get_lambda(interp, 60.0) == 25.0


def test_get_lambda_empty_points() -> None:
...
def test_get_lambda_single_point() -> None:
...
@patch("app.core.material_lambda.Table1D", side_effect=FakeTable1D)
def test_get_lambda_below_range(
    mock_table: MagicMock, valid_material: MockMaterial
) -> None:
    interp = build_lambda_interpolator(valid_material)
    with pytest.raises(
        MaterialPropertyError, match=r"вне диапазона таблицы.*\[20.0; 150.0\]"
    ):
        get_lambda(interp, 10.0)


@patch("app.core.material_lambda.Table1D", side_effect=FakeTable1D)
def test_get_lambda_above_range(
    mock_table: MagicMock, valid_material: MockMaterial
) -> None:
    interp = build_lambda_interpolator(valid_material)
    with pytest.raises(
