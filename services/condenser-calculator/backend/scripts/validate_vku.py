"""
Скрипт автоматизированной валидации расчета Воздушно-Конденсационной Установки (ВКУ).

Использует библиотеку `pandas` для массовой обработки наборов входных данных
(массовый расход, степень сухости, температура воздуха) и сравнения рассчитанного
давления с эталонными (ожидаемыми) значениями.
"""
...
def run_validation():
    """
    Выполняет пакетную проверку математической модели ВКУ на эталонном наборе данных.

    Формирует DataFrame, вызывает стратегию расчета для каждого режима, 
    вычисляет абсолютную и относительную погрешность, и выводит сводную таблицу в консоль.
    """
    print("--- Запуск валидации расчета ВКУ (интерфейс на словарях) ---")

    # [ENGINEERING CONTEXT]
    # В отличие от классических водяных конденсаторов (где используется температура воды),
    # в ВКУ охлаждающей средой является атмосферный воздух. Поэтому здесь ключевым 
    # параметром выступает `temperature_air` (температура наружного воздуха).
    ...
    # [ENGINEERING CONTEXT]
    # Откуда взяты эти массивы G (расход), X (сухость) и P_expected (ожидаемое давление):
    # Это эмпирические данные тепловых испытаний или снятые с реального объекта 
    # режимные характеристики. Они служат "золотым эталоном" (benchmark) 
    # для подтверждения того, что алгоритм расчета работает корректно.
    validation_data = {
    ...
    # [ENGINEERING CONTEXT]
    # Почему выбран порог в 5.0%: 
    # В теплоэнергетических расчетах погрешность до 5% между эмпирическими данными 
    # и аналитической моделью считается инженерно приемлемой (допустимой нормой), 
    # так как учитывает погрешность самих измерительных приборов на электростанции.
