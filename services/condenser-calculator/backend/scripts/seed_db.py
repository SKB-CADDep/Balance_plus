"""
Скрипт инициализации базы данных (Database Seeder).

Заполняет базу данных начальными (эталонными) записями: справочником материалов
и базовыми моделями конденсаторов. Используется для подготовки окружения
при локальной разработке, развертывании тестовых стендов и CI/CD пайплайнах.
Обеспечивает наличие необходимых данных для работы расчетного ядра (DoR).
"""
...
def seed_data(db: Session):
    """
    Выполняет транзакционную вставку или обновление эталонных данных в БД.

    [ENGINEERING CONTEXT]
    Почему используется `db.merge()`, а не `db.add()`: 
    Метод `merge` делает скрипт идемпотентным (безопасным для многократного запуска). 
    Если скрипт запустить дважды, `db.add()` выдаст ошибку уникальности (UniqueViolation) 
    по Primary Key (id). А `db.merge()` проверит: если запись с таким ID уже есть, 
    он просто обновит её поля, если нет — создаст новую.
    ...
    materials = [
        # [ENGINEERING CONTEXT]
        # Почему теплопроводность задана как массив точек (матрица):
        # В термодинамике коэффициент теплопроводности металлов не является константой, 
        # он меняется в зависимости от температуры стенки трубки. Расчетное ядро 
        # использует эти точки для линейной интерполяции теплопроводности "на лету".
        {
            "id": 1,
    ...
    condensers = [
        ...
        },
        {
            ...
            # [ENGINEERING CONTEXT]
            # Почему параметры builtin_* равны None:
            # Далеко не все конденсаторы имеют встроенный теплофикационный пучок 
            # (используемый для подогрева сетевой воды зимой). Данный тип аппарата 
            # является чисто конденсационным, поэтому связанные с пучком поля пустые.
            # Валидаторы и математическое ядро обязаны корректно обрабатывать None.
