...
from app.api.routes.calculations import save_calculation_result, get_latest_calculation
...
class TestSaveCalculationResult:
    ...
    async def test_raises_401_on_authentication_error(self, mock_gitlab: MagicMock):
        ...
        assert "Ошибка авторизации в GitLab" == exc_info.value.detail
    ...
    async def test_raises_404_on_get_error(self, mock_gitlab: MagicMock):
        ...
        assert "Объект не найден в GitLab" == exc_info.value.detail
    ...
    async def test_raises_502_on_gitlab_error(self, mock_gitlab: MagicMock):
        ...
        assert "Ошибка GitLab API: API error" == exc_info.value.detail
