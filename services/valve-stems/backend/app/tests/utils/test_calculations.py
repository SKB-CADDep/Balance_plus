...
class TestValveCalculator(unittest.TestCase):
    def setUp(self):
        p_fresh_mpa = 130 * 0.0980665
        p_intermediate_mpa = 10 * 0.0980665
        p_vacuum_mpa = 1.03 * 0.0980665
        p_suctions_mpa = 0.97 * 0.0980665
        t_start_c = 555.0
    ...
    def test_execute_calculations(self):
        ...
        expected_gi_t_h = [0.5365648691, 0.07402597315, 0.003495929025]
        expected_pi_in_mpa = [12.749, 0.981, 0.101]
        expected_ti_c = [555, 503.6, 40]
        expected_hi_kj_kg = [3487.026558, 3487.026558, 40.24]

        for i in range(len(expected_gi_t_h)):
            self.assertAlmostEqual(result.gi_t_h[i], expected_gi_t_h[i], places=3)
        for i in range(len(expected_pi_in_mpa)):
            self.assertAlmostEqual(result.pi_in_mpa[i], expected_pi_in_mpa[i], places=2)
        for i in range(len(expected_ti_c)):
            self.assertAlmostEqual(result.ti_c[i], expected_ti_c[i], places=1)
        for i in range(len(expected_hi_kj_kg)):
...
class TestValveCalculatorTwo(unittest.TestCase):
    ...
    def test_execute_calculations_two(self):
        ...
        for i in range(len(expected_pi_in_mpa)):
            self.assertAlmostEqual(result.pi_in_mpa[i], expected_pi_in_mpa[i], places=2)
        for i in range(len(expected_ti_c)):
            self.assertAlmostEqual(result.ti_c[i], expected_ti_c[i], places=0)
        for i in range(len(expected_hi_kj_kg)):
            self.assertAlmostEqual(result.hi_kj_kg[i], expected_hi_kj_kg[i], places=0)
