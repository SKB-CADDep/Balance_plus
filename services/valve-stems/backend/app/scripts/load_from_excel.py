...
def init_db():
    ...
    try:
        ...
        for _, row in df_proj.iterrows():
            ...
            all_valves_for_turbine.extend(
                [(v, "Стопорный (СК)") for v in extract_valves(row.get("СК"))]
            )
            all_valves_for_turbine.extend(
                [(v, "Регулирующий (РК)") for v in extract_valves(row.get("РК"))]
            )
            all_valves_for_turbine.extend(
                [(v, "Стопорно-регулирующий (СРК)") for v in extract_valves(row.get("СРК"))]
            )
        ...
        db.commit()
        logger.info(
            f"Успешно загружено! Проектов: {count_turbines}. Всего уникальных клапанов в базе: {len(valve_cache)}"
        )
