"""
Доменные модели предметной области (Domain Models).

Содержит чистые Python-классы (Dataclasses), описывающие физические 
сущности для математического ядра (геометрия клапанов, термодинамика). 
Эти классы полностью изолированы от веб-фреймворка (FastAPI) и базы данных (SQLAlchemy).

Ключевой принцип: строгая фиксация единиц измерения (СИ).
"""
