...
class CalculationAdapter:
    ...
    def _process_single_group(
        ...
        # АВТОМАТИЧЕСКАЯ ГЕНЕРАЦИЯ ДАВЛЕНИЙ (p_values)
        p_in_mpa = [p_fresh_mpa]  # P0 всегда свежий пар
        for i in range(count_parts - 1):
            if i < len(p_suctions_mpa):
                p_in_mpa.append(p_suctions_mpa[i])
            else:
                p_in_mpa.append(p_lst_mpa) # Фоллбэк на вакуум
