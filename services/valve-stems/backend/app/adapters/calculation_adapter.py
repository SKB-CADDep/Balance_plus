...
class CalculationAdapter:
    ...
    def _process_single_group(
        ...
        user_inputs_mpa = [
            converter.convert(p, from_unit=group_in.p_leak_offs_unit,
        ...
        # Конвертация ответа
        pi_out = [converter.convert(p, from_unit="МПа", to_unit="кгс/см²",
                                    parameter_type="pressure") for p in raw.pi_in_mpa]
        dea_p_out = converter.convert(raw.dea_p_mpa, from_unit="МПа",
                                      to_unit="кгс/см²", parameter_type="pressure") if raw.dea_p_mpa else 0.0

        ej_props_out = []
        for ej in raw.ej_results:
            ej_p_out = converter.convert(ej["p_mpa"], from_unit="МПа",
                                         to_unit="кгс/см²", parameter_type="pressure") if ej["p_mpa"] else 0.0
    ...
    def run_multi_calculation(
        ...
        try:
            p_fresh_mpa = converter.convert(globals_data.P_fresh, from_unit=globals_data.P_fresh_unit,
                                            to_unit="МПа", parameter_type="pressure")
            t_air_c = converter.convert(globals_data.T_air, from_unit=globals_data.T_air_unit,
                                        to_unit="°C", parameter_type="temperature")
            p_lst_mpa = converter.convert(globals_data.P_lst_leak_off,
            ...
            if globals_data.T_fresh is not None:
                ...
                h_start_kj = converter.convert(globals_data.H_fresh, from_unit=globals_data.H_fresh_unit,
                                               to_unit="кДж/кг", parameter_type="enthalpy")
