...
def scan_for_test_groups(root_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        try:
            with open(
                os.path.join(dirpath, "__group__.yml"), "r", encoding="utf-8"
            ) as f:
        ...
        for file in filenames:
            if file.startswith("test_") and (
                file.endswith(".py") or file.endswith(".yaml")
            ):
                ...
                if file.endswith("_calc.py"):
                    ...
                    file_icon = (
                        "🌐" if "tavern" in file else "🗄️" if "db" in file else "📄"
                    )
...
def find_project_root(test_path):
    ...
    while current != os.path.dirname(current):
        if os.path.exists(os.path.join(current, "pytest.ini")) or os.path.exists(
...
def main_menu():
    ...
    while True:
        ...
        log_status = (
            f"{Colors.GREEN}ВКЛ{Colors.RESET}"
            if SAVE_LOGS
            else f"{Colors.GRAY}ВЫКЛ{Colors.RESET}"
        )
        print(
            f"[{Colors.GREEN}l{Colors.RESET}] Сохранять логи при ошибках: {log_status}"
        )
